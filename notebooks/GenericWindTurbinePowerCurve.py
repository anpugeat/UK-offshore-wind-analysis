# -*- coding: utf-8 -*-
"""
Python file with functions for all notebooks to use.
"""
...
def CpLambdaModels(Model,TSR,Beta=[]):
    ...
    #Sources=['Slootweg et al. 2003','Heier 2009','Thongam et al. 2009','De Kooning et al.  2010','Ochieng et Manyonge 2014','Dai et al. 2016','constant']
...
def WT_PowerCurve_raw(Vws,Pnom,Drotor,rMin=[],rMax=[],CpMax=[],Model='Dai et al. 2016',Beta=[],AirDensity=1.225,ConvEff=0.92):
    ...
    # ********************************************************************************************
    if isinstance(rMin, list) and len(rMin) == 0:
        rMin=188.8*Drotor**(-0.7081)   # minimal angular speed in rpm
    if isinstance(rMax, list) and len(rMax) == 0:
    ...
    if isinstance(CpMax, list) and len(CpMax) == 0:
        CpScale=1
        CpMax=max(vCp)
    else:
...
def GenericWindTurbinePowerCurve(Vws,Pnom,Drotor,zhub=[],Vcutin=3,Vcutoff=25,TI=0.1,Shear=0.15,Veer=0,rMin=[],rMax=[],CpMax=[],Model='Dai et al. 2016',Beta=[],AirDensity=1.225,ConvEff=[]):
    ...
    # conversion losses
    if isinstance(ConvEff, list) and len(ConvEff) == 0:
        gear_loss_const= .01
        gear_loss_var= .014
        generator_loss= 0.03
        converter_loss= .03
    ...
    if isinstance(zhub, list) and len(zhub) == 0:
